...
export default function HonorarkraftEinsatzDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  return (
    <RoleGuard allowedRoles={["Honorarkraft"]}>
      <HonorarkraftEinsatzDetailsContent einsatzId={id} />
    </RoleGuard>
  )
}

function HonorarkraftEinsatzDetailsContent({ einsatzId }: { einsatzId: string }) {
